...
;(function addCustomStyles() {
    ...
    .double .custom-armory-bonus-badge:first-child {
      top: 0 !important;
      bottom: 50% !important;
    }
    .double .custom-armory-bonus-badge:last-child {
      top: 50% !important;
      bottom: 0 !important;
    }`
...
function inventoryandbazaar(triggered) {
	...
	if (link.includes("item")) {
		...
		if (triggered && triggered[0] && triggered[0].childElementCount >= 1) {
			...
			let container =
				triggered[0].parentElement.parentElement.parentElement
...
function newItemMarket(triggered) {
	...
	if (secondBonusExists) {
		...
		const desc2 = secondary.getAttribute(
